...
namespace Urg
{
    public class DebugRenderer : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (rawDistances != null && rawDistances.Length > 0)
            {
                for (int i = 0; i < rawDistances.Length; i++)
                {
                  ...
                  //  Debug.DrawRay(urg.transform.position, pos, Color.red);
                   // Debug.Log(pos);
                    GameObject a =Instantiate(gb, pos, Quaternion.Euler(0,0,0));
                    Destroy(a,0.5f);
                }
            }
        ...
        {
          //  Debug.Log("distance received: SCIP timestamp={0} unity timer={1}", data.Timestamp, stopwatch.ElapsedMilliseconds);
